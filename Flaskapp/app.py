...
def graphcall():
    ...
    # agendaTitel = graph_data['bodyPreview']
    # send_to_servicebus(agendaTitel)
    # service_bus_data = get_from_servicebus()
    # print(service_bus_data)

@app.route("/egraphcall")
def egraphcall():
    token = _get_token_from_cache(app_config.SCOPE)
    if not token:
        return redirect(url_for("login"))
    graph_data = requests.get(
        app_config.EENDPOINT,
        headers={'Authorization': 'Bearer ' + token['access_token']},
        ).json()
